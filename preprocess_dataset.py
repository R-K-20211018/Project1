...
def find_dis(point):
    ...
    dis = np.mean(np.partition(dis, kth, axis=1)[:, 1:4], axis=1, keepdims=True)
    return dis

def generate_data(im_path):
    im = Image.open(im_path)
    im_w, im_h = im.size
    mat_path = im_path.replace('.jpg', '.mat').replace('images','ground-truth').replace('IMG_','GT_IMG_')
...
def parse_args():
    ...
    parser.add_argument('--origin-dir', default='./original-data',
